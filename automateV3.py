...
def on_message(client, userdata, msg):
    ...
    ambientLight = int(msg.payload)
    if(ambientLight < 30):
        cloud_state[0] = 0
        cloud_state[1] = 0
    elif(ambientLight < 50):
        cloud_state[0] = 0
        cloud_state[1] = 1
    else:
        cloud_state[0] = 1
...
while True:
	try:
		...
		if isinstance(humi, float) and isinstance(temp, float):
			...
			conn = urllib2.urlopen(baseURL + '&field1=%s&field2=%s&field3=%s&field4=%s&field5=%s&field6=%s' % (temp, humi,ambientLight,cloud_state[0],cloud_state[1],cloud_state[2]))
			print conn.read()
			print ambientLight
